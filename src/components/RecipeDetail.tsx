...
const RecipesDetails: React.FC<RecipesDetailsProps> = ({ recipe }) => {
  return (
    <div className="w-full max-w-3xl mx-auto p-4 sm:p-6 bg-white shadow-md rounded-lg relative border border-orange-100 mb-20 mt-10">
      ...
      <div className="flex flex-col md:flex-row gap-6 mb-6">
        <div className="relative w-full sm:w-64 h-64 mb-6 md:mb-0">
          <Image
            src={recipe.image}
            alt={recipe.name}
            layout="fill"
        ...
        <div className="md:w-1/2 md:pl-6 flex flex-col justify-center">
          ...
          >
            {recipe.ingredients.map((ingredient, index) => (
              <li
                key={index}
                className="text-gray-700 text-base sm:text-lg mb-1"
              >
                {ingredient}
              </li>
            ))}
